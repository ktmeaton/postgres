...
psql -U postgres -f sql/setup/_all.sql postgres | tee -a ${PGDATA}/log/init.log

echo "Performing setup in $CUSTOM_DB database"
psql -U postgres -f sql/_all.sql $CUSTOM_DB 2>&1 | tee -a ${PGDATA}/log/init.log
